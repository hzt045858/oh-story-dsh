...
2. A `short-drama` Skill provider for 11 pinned Drama Skills.
...
`packages/knowledge/oh-story/manifest.json` pins the Oh Story release, commit, 13 Skills, 7 Roles, agents version and every file hash. `packages/knowledge/drama/manifest.json` performs the same role for 11 Drama Skills. `packages/knowledge/novel-to-game/manifest.json` pins NovelToGame 0.3.1, all 7 Skills and the complete playable `jin-ping-mei` example. `packages/knowledge/video-recap/manifest.json` pins video-recap-skills 0.5.0, all 6 Skills and every bundled file hash.
