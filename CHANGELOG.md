...
- 新增**公众号创作工作台**与自研 Skill `/wechat-article`，五套工作台之外的第一套自有知识资产。按 `公众号/<账号>/` 组织文章、风格文件与排版成果，支持 Markdown/HTML 源码编辑、手机/桌面预览、本地配图、版本校验保存与未保存草稿恢复；`参考文章/` 下的原文在编辑器和 PUT 路由两侧都只读，预览禁用脚本和外部资源，不会自动上传或发布。工作流先全量阅读参考文章建立文章索引、分主题与全局/图文风格模型，之后按「新选题 + 已保存风格模型」写作，不再回读原文；图片型账号额外分析图片文字、画面、图文关系与多图结构，OCR 仅辅助读字，缺少实际正文图片或尚未完成图文审稿时保持待生成/待审状态——文字稿和提示词不算图片文章完成。已内置账号注册、发布准备、封面/正文图片上传、草稿创建与更新、指定接收者预览、公开发布、按标签/全体群发、状态查询与回执恢复：公开发布与群发分别处理，提交任务不会被当作已完成，批次可设置带时区的最早发布时间。微信账号配置只保存 AppID 与密钥的环境变量名，各账号的素材、草稿与回执互相隔离；实际调用仍需微信接口权限和 IP 白名单。六个第三方参考项目已登记固定版本，MIT/Apache 的按许可随包分发、AGPL/受限项目的对应功能自行实现。方法来源、17 节要求映射与适配说明见 `packages/knowledge/wechat/skills/wechat-article/references/sources.md`。
- 新增 **Tauri 2 Windows 桌面发行版**。沿用小说、短剧、游戏、视频、公众号五个工作台，随包携带 Node.js 24、DSH 与本插件，自动启动本地 DSH，支持托盘驻留，并用独立数据目录保存会话、配置与草稿。源码启动用 `pnpm desktop`，便携版用 `pnpm desktop:portable`，安装包用 `pnpm desktop:build`；构建需要 Windows x64、Node.js 24、Rust、Visual Studio C++ Build Tools 与 WebView2。便携版必须保留 EXE 旁的 `runtime` 目录；模型凭据、Python 与 ffmpeg 等制作依赖仍需另行配置。**浏览器里已存在的未保存草稿不会自动转入桌面版**，切换前请先保存到作品文件。完整说明见 [桌面版说明](docs/DESKTOP.md)。
- 小说与短剧编辑器自动把**未保存草稿**备份到浏览器 `localStorage`，按 origin、workspace cwd 与 Session ID 隔离，只备份每个脏文件的 `content`、`saved` 基线与文件系统 `version`（不含编辑器导航与生产面板偏好）。页面刷新或 Host 重启后重新打开原 Session 即可恢复：磁盘文件被改动会提示冲突，被删除的文件仍可下载或丢弃草稿，另一个页面同时在编辑时会询问创作者而不是互相覆盖，存储失败提供重试与下载。备份不跨浏览器或 origin（含端口）迁移，清除站点数据即丢失。写入者按 workspace/Session 持有独占的浏览器 Web Lock，没有 Web Locks 的浏览器会报告备份不可用。草稿首次变脏时 `POST /oh-story/draft-session` 请求 DSH 的持久化检查点保住这条活动 Session（包括空会话），不产生 chat 事件、不写创作文件；只有编辑文档的创作者因此也能在 Host 重启后接着写。
- 生产文档接入与源码编辑同一套缓冲、版本校验与草稿备份；写盘仍是以文件系统版本为前置条件的显式人工动作，过期写入被拒绝而不是覆盖并发改动。
- 空目录新增**工作台入口引导**：选择作品目录后创建并连接 workspace、打开会话，并把选定的工作台模式随导航一起发布，DSH 复用空白会话时也不会丢失。

### Fixed

- 视频工作台的预览选择不再因为项目里暂时没有匹配的预览项就把已载入的视频换成空状态，`selected` 为空时「载入新版本」按钮也不再出现；成片选择改为随项目数据重新解析，不再停留在已失效的条目上。预检新增「载入中」状态，并发请求会被后一次取消。
- 视频工作台的操作图标补上 16×16 尺寸，不再随容器拉伸变形。
...
- 打包与 lint 的两条护栏此前把上游资产误当成本仓库的代码。`short-drama-edit` 随包附带一条可选的 Remotion 字幕排版路线，它带自己的 `src/` 与 TSX 文件：发布包的「不得包含源码」检查改为只拒绝本插件自己的 `package/src/` 与 `package/tests/`，ESLint 与 `tsconfig.lint.json` 则整体排除 `packages/knowledge/**`——上游资产逐字随包交付并由 parity 校验哈希，本仓库不该改它的写法。
- 原生集成测试定位插件 Browser 模块的方式改为容忍压缩与未压缩两种注册写法。合并后的 Browser bundle 两种形式都有，此前只按压缩写法找下一个模块边界，切片会一路越过插件、把紧随其后的 DSH 模块也算成插件代码（`MutableSessionEventSource` 因此被当成遗留的 `EventSource` 接口）。
