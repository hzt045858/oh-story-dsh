...
const DSH_DRAMA_BRIDGE = [
  ...
  "剧集/<EP>/剪辑单.md is the v0.7 assembly record, not a sixth creative truth. It is written after footage exists and records only which frames of that footage reach the cut; changing a line, a shot's job, or a declared duration means returning to the document that owns it.",
...
const DSH_DRAMA_OVERRIDES: Readonly<Partial<Record<string, string | ((skillRoot: string) => string)>>> = {
  ...
  "short-drama-edit": "Assembly runs inside this DSH Session. Write 剧集/<EP>/剪辑单.md first, then run edit_tool.py check and render through the current DSH execution world; the rendered cut belongs under 剧集/<EP>/制作成果/成片/ and the 短剧 tab shows it there. ffmpeg and ffprobe come from the DSH host — when one is missing, say so instead of approximating or reporting an untested measurement as a pass. Keep the zero-dependency ffmpeg subtitle route by default; the Remotion route installs Node dependencies and renders every frame through a headless browser, so take it only after the creator approves that install, and never treat it as a second Agent runtime, dashboard, or session transport. This stage never generates footage and never edits 剧本.md, 分镜.md, or 视频提示词.md — route those back to their owning Skill.",
