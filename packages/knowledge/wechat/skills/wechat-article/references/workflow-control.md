...
```json
{
  ...
  "closing":"",
  "cards":[{
    "id":"body-01","order":1,"role":"body","prompt_compiler_version":2,
...
新图位设置 `prompt_compiler_version: 2`：编译器优先传递本图精确文字、场景和执行规则，
来源证据仍保存在锁定契约中，不混入生图提示词。缺省值1保留旧提示词和回执兼容性。
已有成功图无需仅为升级编译器重做；更改某图的编译版本时审核下一修订并换输出版本。
