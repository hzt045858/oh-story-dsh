...
首次分析图片正文时，先按 [图片正文分析方法](image-body-analysis.md) 阅读全部纳入文章，
检查单图文字与画面、声音归属、整组结构、图外段落及封面，再提炼规则。用户要求更新风格时只补充新增、变更资料及受影响证据；
模型完成后的日常创作直接使用已保存规则，不重新读原文或原图。本文件负责证据结构与执行契约。
...
文字文章若全部图片经观察确认为身份、广告或装饰项，仍需记录每图排除依据和整篇 sequence；
此时 body_orders、transitions、transfer_rules 均为空数组，可以验收文字分析，但不能据此提炼视觉规则。
模型验收分别统计含图片引用的 image_articles 和含已复核正文图的 body_image_articles；
只有后者要求联合视觉模型，未完成图片复核的文章仍保持待处理，不会被自动排除。
